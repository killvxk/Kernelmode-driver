...
DWORD64 initProcess(HANDLE processID, HANDLE clientProcessID, communicationStruct* systemBuffer)	/*		setup required info	(and sig scan), find read output buffer		*/
{
	...
	DWORD64		clientBaseAddress = (ULONG64)GetUserModule(clientProcess, &clientprocessName, isclientWow64); //BSOD (sometimes)
...
BOOLEAN PlaceEverythingHook()
{
	...
	ULONG64		returnPlaceOfIOCTL	 =	 (ULONG64)diskSysBase + 0x112E;
...
BOOLEAN PlaceDiskHook()
{
	...
	//DiskIoctlVerify
	ULONG64		hookLocation = (ULONG64)diskSysBase + 0x2D90;
